...
try {
 ...
 if(!publicMode){
  ...
  await page.goto(access.href);violations.push(...await page.evaluate(()=>window.ownedCspViolations));
 }
 await page.goto(base);
 await expect(page.getByRole('heading',{level:1})).toContainText('Make your console');
 await expect(page.locator('.fine-print').filter({hasText:'Core and React 0.1.0 are available on npm'})).toBeVisible();
 ...
 await expect.poll(()=>page.evaluate(()=>localStorage.getItem('console-fx:scene:v1'))).toContain('Keep this draft');violations.push(...await page.evaluate(()=>window.ownedCspViolations));await page.reload();await expect(text).toHaveValue('Keep this draft');
 ...
 violations.push(...await page.evaluate(()=>window.ownedCspViolations));
 await page.goto(base+'/docs/');await expect(page.locator('#integration pre code').first()).toHaveText('pnpm add @servrox/console-fx@next');await expect(page.locator('#react pre code').first()).toHaveText('pnpm add @servrox/console-fx@next @servrox/console-fx-react@next');await expect(page.getByRole('heading',{level:1})).toContainText('Use ConsoleFX');violations.push(...await page.evaluate(()=>window.ownedCspViolations));
